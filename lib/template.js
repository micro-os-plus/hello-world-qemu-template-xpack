...
export class XpmInitTemplate {
  ...
  constructor(context) {
    assert(context, 'The context parameter is mandatory.')
  ...
  async run() {
    ...
    const mustAsk = Object.keys(propertiesDefinitions).some((key) => {
      return propertiesDefinitions[key].isMandatory && !config.properties[key]
    })
    ...
    if (mustAsk) {
      ...
      Object.entries(propertiesDefinitions).forEach(([key, val]) => {
  ...
  async copyFolderRecursive(sourcePath, destinationPath) {
    ...
    await makeDirectory(path.dirname(destinationPath))
  ...
  async generate(liquidMap, isInteractive) {
    ...
    await makeDirectory(config.cwd)
    ...
    await this.copyFile('dot.vscode/tasks.json', '.vscode/tasks.json')
    await this.copyFile('dot.vscode/settings.json', '.vscode/settings.json')
    await this.copyFile('dot.clang-format', '.clang-format')
